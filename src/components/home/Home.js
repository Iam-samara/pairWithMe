...
var Home = React.createClass({
	...
	render: function() {
		...
		var projectList = this.state.projects.map(function(element, index){
			return (<Projects className="col-xs-12 col-sm-4" title={element.projectName.slice(0,17)} description={element.description.slice(0,17)} tools={element.tools.slice(0,17)} route={element.id} key={index}/>)
		});
		return (
		<div>
			<div className="row">
